...
function skyscannerCity(cityCode) {
    return fetch("https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/autosuggest/v1.0/US/USD/en-US/?query=" + cityCode, {
        "method": "GET",
        "headers": {
        ...
        .then(function (response) {
            ...
            `
                ...
                for (i = 0; i < data.Carriers.length; i++) {
                    if (data.Quotes[0].OutboundLeg.CarrierIds[0] == data.Carriers[i].CarrierId) {
                        var row2 = `
                <tr>
                <td>${from}</td>
                <td>${to}</td>
